...
To assess whether a prompt is out of distribution (OOD), we run
membership_inference/Membership_Inference_script.py five times, each time using a different seed image (indexed from full_0001.png to full_0005.png). The script is executed using the following command:
```python
